...
const discountPrices = (prices, discounts) => {
    let newPrices = []; //create a variable to store the new prices
    for (let i = 0; i < prices.length; i++) { //loop through the prices
        newPrices.push('$' + (prices[i] * (1 - discounts[i])).toFixed(2)); //add the new price to the array
    }
    return newPrices; //return the new array
}

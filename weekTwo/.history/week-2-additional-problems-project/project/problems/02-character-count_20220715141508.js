...
function characterCount(string) {
    ...
    for (let i = 0; i < string.length; i++) { //loop through the string
        let letter = string[i]; //create a variable to store the letter
        if (obj[letter]) { //if the letter is in the object
            obj[letter]++; //increment the value
        } else { //if the letter is not in the object

...
function interrupter(interruptingWord) {
    return function(string) { //return a function that takes in a string
        ...
        for (let i = 0; i < words.length; i++) { //loop through the words
            if (words[i] === interruptingWord) { //if the word is the interrupting word
                words[i] = interruptingWord + " " + interruptingWord;   //replace the word with the interrupting word + " " + interrupting word
            }
        }
        return words.join(" "); //return the joined words
    }
}

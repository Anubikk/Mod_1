...
const coinCollector = (numCoins) => {
  let coins = []; //create a variable to store the coins
  return function(coin) { //return a function
    coins.push(coin); //add the coin to the array
    if (coins.length === numCoins) { //if the length of the array is equal to the number of coins
      return coins; //return the array
    } else { //if the length of the array is not equal to the number of coins
      return coinCollector(numCoins); //return the function
    }
  }
}

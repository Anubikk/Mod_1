...
const curriedSum = (numArgs) => {
  ...
  let _curriedSum = (num) => { //create a function
    numbers.push(num); //append the number to the array
    if (numbers.length === numArgs) { //if the array has the correct number of elements
      return numbers.reduce((a, b) => a + b); //return the sum of the array
    } else { //else
      return _curriedSum;  //return itself
    }
  }
  return _curriedSum; //return the function
}

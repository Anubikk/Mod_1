...
function pickyMyMap(array, callback) {
	...
	for (let i = 0; i < array.length; i++) {//loop through the array
		let resultOfCallback = callback(array[i]);//call the callback function and store the result in a variable
		if (resultOfCallback) {//if the result of the callback function is truthy
			result.push(resultOfCallback);//add the result to the result array
		}
	}

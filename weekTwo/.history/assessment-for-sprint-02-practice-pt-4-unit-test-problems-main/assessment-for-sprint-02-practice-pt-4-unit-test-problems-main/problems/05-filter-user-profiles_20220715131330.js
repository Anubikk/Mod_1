...
***********************************************************************/
function filterUserProfiles(userProfiles, callback) {
  let result = [];//create an empty array to store the results of the callback function
  for (let i = 0; i < userProfiles.length; i++) {//loop through the array
    let results = callback(userProfiles[i]);//call the callback function and store the result in a variable
    if (results) {//if the result of the callback function is truthy
      result.push(userProfiles[i].name);//add the result to the result array
    }
  }

...
function duplicateCharMinCount(string, minCount) {
	...
	for (let i = 0; i < string.length; i++) {//loop through the string
		if (letterCount[string[i]] === undefined) {//if the character is not in the object
			...
			letterCount[string[i]]++;//increment the count of the character
		}
	}
	for (let key in letterCount) {//loop through the object
		if (letterCount[key] >= minCount) {//if the count of the character is greater than or equal to minCount
			lettersArray.push(key);//add the character to the array
		}
	}
	return lettersArray;//return the array
}

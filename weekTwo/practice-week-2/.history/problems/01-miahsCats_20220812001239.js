...
// in the input object containing the string "cute" or "adorable".
 const miahsCats = (obj) => {
        let count = 0;
        if (obj.hasOwnProperty('cute').toLowerCase() || obj.hasOwnProperty('adorable').toLowerCase()) {
            count++;
        }
        if (obj.hasOwnProperty('adorable').toUpperCase() || obj.hasOwnProperty('cute').toUpperCase()) {
            count++;
        }
        return count;
    }

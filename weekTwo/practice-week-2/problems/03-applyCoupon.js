...
const applyCoupon = (discount, wishList) => {
    let total = 0; //create an empty variable to store the total
    for (let i = 0; i < wishList.length; i++) { //loop through the array
        let price = wishList[i].price; //get the price of the item
        let discountPrice = price - (price * discount / 100); //get the discount price
        total += discountPrice; //add the discount price to the total
    }

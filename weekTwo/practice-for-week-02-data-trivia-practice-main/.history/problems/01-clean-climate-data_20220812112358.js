...
//1) the object contains the 'city', 'country', and 'monthlyAvg' keys
//2) returns the correct object for the second instance of the duplicated city

const returnDuplicate = (climateData) => {
    let dupClimateData = findDuplicates(climateData)
    let dupCity = Object.keys(dupClimateData)[0]
    let dupId = dupClimateData[dupCity][dupClimateData[dupCity].length - 1]
    for (let i = 0; i < climateData.length; i++) {
        if (climateData[i].id === dupId) {
            return climateData[i]
        }
    }
}
...
const correctDuplicate = (climateData, newCity) => {
    for (let i = 0; i < climateData.length; i++) {
        if (climateData[i].city === newCity) {
            climateData[i].city = newCity
        }
    }
    return climateData
}

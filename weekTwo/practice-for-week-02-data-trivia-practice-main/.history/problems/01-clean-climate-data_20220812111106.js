...
// ✔ returns an object
// ✔ the object contains the 'city', 'country', and 'monthlyAvg' keys
// 5) returns the correct object for the second instance of the duplicated city
...
const correctDuplicate = (climateData, newCity) => {
    let duplicates = findDuplicates(climateData)
    let duplicate = Object.keys(duplicates).find(key => duplicates[key].length > 1)
    let duplicateIndex = climateData.findIndex(city => city.city === duplicate)
    climateData[duplicateIndex].city = newCity
    return climateData[duplicateIndex]
}

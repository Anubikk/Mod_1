...
// 1) returns an object
// 2) the object contains the 'city', 'country', and 'monthlyAvg' keys using .find()
// 3) returns the correct object for the second instance of the duplicated city

const returnDuplicate = (climateData) => {
    let reduplicate = findDuplicates(climateData)
    for (let key in reduplicate) {
        if (reduplicate[key].length > 1) {
            return climateData.find(city => city.id === reduplicate[key][1])
        }
    }
}
...
// 4) returns the correctly-formatted output with the new city name
 ...
 const correctDuplicate = (climateData) => {
    let singleDuplicate = returnDuplicate(climateData)
    for (let i = 0; i < climateData.length; i++) {
        if (climateData[i].id === singleDuplicate[0]) {
            climateData[i].city = singleDuplicate[1]
        }
    }

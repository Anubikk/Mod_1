...
const returnDuplicate = (findDuplicates) => {
    let duplicate = {}
    for (let key in findDuplicates) {
        if (findDuplicates[key].length > 1) {
            duplicate[key] = findDuplicates[key][findDuplicates[key].length - 1]
        }
    }
    return duplicate
}
...
//         }
//     }).sort()
//     duplicate[newCity].city = newCity
//     return duplicate
// }

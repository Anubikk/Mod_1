...
const citiesWithMostSnowDays = (climateData) => {
    let maxRain = climateData.map(city => city.totalSnowDays).reduce((a, b) => Math.max(a, b));
    let cities = [];
    for (let i = 0; i < climateData.length; i++) {
        if (climateData[i].totalSnowDays === maxRain) {
            cities.push({
                city: climateData[i].city + ', ' + climateData[i].country,
                totalSnowDays: climateData[i].totalSnowDays
            });
        }
    }
    return cities;

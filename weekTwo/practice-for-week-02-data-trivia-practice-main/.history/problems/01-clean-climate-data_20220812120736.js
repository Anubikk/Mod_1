...
const returnDuplicate = (climateData) => {
    ...
    for (let key in reduplicate) {
        if (reduplicate[key].length > 1) {
            return climateData[reduplicate[key][reduplicate[key].length -1]]
        }
    }
}
...
const correctDuplicate = (climateData) => {
    let oneDupe = returnDuplicate(findDuplicates(climateData))
    for (let i = 0; i < climateData.length; i++) {
        if (climateData[i].id === oneDupe.id) {
            climateData[i].city = oneDupe.city
        }
    }
    return oneDupe

...
const citiesWithMostSnowDays = (climateData) => {
    let cities = climateData.map(city => {
        return {
            city: `${city.city}, ${city.country}`,
            totalSnowDays: city.rainfall.reduce((a, b) => a + b).filter(day => day === 1).length
        }
    }).sort((a, b) => b.totalSnowDays - a.totalSnowDays)
    return cities.filter(city => city.totalSnowDays === cities[0].totalSnowDays)
}

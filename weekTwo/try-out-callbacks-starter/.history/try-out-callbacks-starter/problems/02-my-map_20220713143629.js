...
let myMap = function(array, cb) {
    ...
    for (let i =0; i < array.length; i++) { //create a for loop to iterate through array
        let ele = array[i]; //set a var to the index of the array
        map.push(cb(ele)); //push new elements with cb func to new array
    }
    return map;//return new array
};

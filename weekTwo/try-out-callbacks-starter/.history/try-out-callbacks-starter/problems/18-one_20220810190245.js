...
const one = function(array, cb) {
    return array.some(cb) && array.filter(cb).length === 1; //returns true if one element of the array results in true when passed into the callback
}

...
let mySimpleReduce = function(array, callback) {
    let accumulator = array[0]; // set the accumulator to the first element of the array
    for (let i = 1; i < array.length; i++) { // loop through the array starting at the second element
        accumulator = callback(accumulator, array[i]); // call the callback with the accumulator and the current element
    }
    return accumulator;

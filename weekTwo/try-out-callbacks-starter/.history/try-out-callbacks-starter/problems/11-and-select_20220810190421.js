...
let andSelect = function(array, cb, cb2) {
    let newarray = []; // create new array to hold filtered elements
    for (let i = 0; i < array.length; i++) { // loop through array
        let ele = array[i]; // set ele to current element in array
        if ((cb(ele)===true) && (cb2(ele)===true)) { // if element passes cb and cb2
            newarray.push(ele); // push element into new array
        }
    }
    return newarray;
};

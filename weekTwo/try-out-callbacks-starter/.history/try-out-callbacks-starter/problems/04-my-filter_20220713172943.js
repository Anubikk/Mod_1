...
let myFilter = function(array, cb) {
    let newArr = []; //create new array to push elements
    array.forEach(function(ele, i){  //create forEach to iterate through array by index
       if (cb(ele, i)){ //if the index of the element is true push to the new array
           newArr.push(ele)
       }
    });
    return newArr

...
const { forEach } = require('./01-fruit-data-structure');
...
function updateKeyName(array, oldKey, newKey) {
    return array.map(fruit => {
        const oldValue = fruit[oldKey];
        delete fruit[oldKey];
        fruit[newKey] = oldValue;
        return fruit;
    });
}
...
*/
//returns the correct output with all `id` values formatted correctly to 6 digits
function updateIdValues(array) {
   return array.map(fruit => {
       fruit.id = fruit.id.toString().padStart(6, '0');
       return fruit.id;
   });
}

...
function advancedExponent(b, n) {
  ...
  if (n < 0) { // if power is negative, divide by the exponent
    return 1 / (b * advancedExponent(b, n + 1));
  }
  if (n % 2 === 0) { // if power is even, square the result of the exponent
    return advancedExponent(b, n / 2) ** 2; // recursive call with n / 2 and n / 2 (even n)
  }
  return b * (advancedExponent(b, (n - 1) / 2) ** 2); // if the power odd recursive call with n - 1 and n - 2 (odd n)
}

//second attempt one line - recursive
const advancedExponent = (b, n) => n === 0 ? 1 : n < 0 ? 1 / (b * advancedExponent(b, n + 1)) : n % 2 === 0 ? advancedExponent(b, n / 2) ** 2 : b * (advancedExponent(b, (n - 1) / 2) ** 2);

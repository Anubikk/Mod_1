...
//second try one-liner - recursive
const subsets = (arr) => arr.length === 0 ? [[]] : arr.length === 1 ? [[], [arr[0]]] : subsets(arr.slice(1)).map((subset) => subset.concat(arr[0])).concat(subsets(arr.slice(1)));

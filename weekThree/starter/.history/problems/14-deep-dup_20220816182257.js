...
//     return [];
//   }
//   if (Array.isArray(arr[0])) { // if the first element is an array, call deepDup on the first element and concatenate it with the rest of the array
//     return [deepDup(arr[0])].concat(deepDup(arr.slice(1))); // recursive call with the first element removed from the array and concatenate it with the rest of the array
//   }
//   return [arr[0]].concat(deepDup(arr.slice(1))); // recursive call with the first element removed from the array and concatenate it with the rest of the array
// }

//second attempt one-liner - recursive
const deepDup = (arr) => arr.length === 0 ? [] : Array.isArray(arr[0]) ? [deepDup(arr[0])].concat(deepDup(arr.slice(1))) : [arr[0]].concat(deepDup(arr.slice(1)));

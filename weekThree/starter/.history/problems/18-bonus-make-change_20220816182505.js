...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  for (let i = 0; i < coins.length; i++) {
    ...
    if (remainder >= 0) {
      let change = greedyMakeChange(remainder, coins.slice(i));
      if (change !== null) {
        change.push(coins[i]);
...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
 if (target === 0) {
  ...
  for (let i = 0; i < coins.length; i++) {
    let remainder = target - coins[i];
    if (remainder >= 0) {
      let change = makeBetterChange(remainder, coins.slice(i));
      if (change !== null) {
        change.push(coins[i]);
        if (bestChange === null || change.length < bestChange.length) {
          bestChange = change;
...
const makeBetterChange = (target, coins = [25, 10, 5, 1]) => remainder >= 0 ? makeBetterChange(remainder, coins.slice(i)) : null;

const greedyMakeChange = (target, coins = [25, 10, 5, 1]) => remainder >= 0 ? greedyMakeChange(remainder, coins.slice(i)) : null;

...
***********************************************************************/
//with a simple example it should build a tree with the specified categories
// with a complicated example it should build a tree with the specified categories
const makeTree = (categories, parent) => {
  if (categories === null) { // if the categories array is empty, return an empty object
    return {};
  }
  if (categories.length === 0) { // if the categories array is empty, return an empty object
    return {};
  }
  if (categories[0].parent === parent) { // if the first element in the array has the same parent as the provided parent, return an object with the id as the key and the rest of the array as the value
    return {[categories[0].id]: makeTree(categories.slice(1), categories[0].id)};
  }
  return {[categories[0].id]: makeTree(categories.slice(1), parent)}; // if the first element in the array has a different parent than the provided parent, return an object with the id as the key and the rest of the array as the value
}
